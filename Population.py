...
class Population:
    ...
    def update_stats(self):
        for p in self._population:
            p.compute_makespan_1()
            p.compute_release_date()
            p.compute_makespan_2()
    ...
    def create_initial_population(self):
        ...
        self._population[4].apply_random_rule_1()
        self._population[5].apply_random_rule_1()

        for i in range(6):
            self._population[i].compute_makespan_1()
            self._population[i].compute_release_date()

        self._population[0].apply_lpt_rule_2()
        self._population[1].apply_spt_rule_2()
        self._population[2].apply_lns_rule_2()
        self._population[3].apply_sns_rule_2()
        self._population[4].apply_sns_rule_2()
        self._population[5].apply_sns_rule_2()
        '''
        ...
        for i in range(0, len(self._population)):
            ...
            '''
            ordering = random.choice(ordering_second_stage_options)

            if ordering == 'LPT':  # LPT
                self._population[i].apply_lpt_rule_2()
            elif ordering == 'SRT':  # SRT
                self._population[i].apply_spt_rule_2()
            elif ordering == 'LNS':  # LNS
                self._population[i].apply_lns_rule_2()
            elif ordering == 'SNS':  # SNS
                self._population[i].apply_sns_rule_2()
            elif ordering == 'AVA1':  # SNS
                self._population[i].apply_sns_rule_2()
            elif ordering == 'AVA2':  # SNS
                self._population[i].apply_sns_rule_2()
            else:  # Random
                self._population[i].apply_random_rule_2()
            '''
